...
int main() {
    ...
    // we could have directly saved the sum inside the thread struct 
    // but no!! we have made things complicated just for the questions sake XD
    for (int i=0; i<LEN; i++) {
        pthread_create(&(thread[i].thread_id), NULL, asciiSum, (void *)&(thread[i]));
        pthread_join(thread[i].thread_id, (void **)&ret);
        thread[i].sum = *ret;
        free(ret);
    }
